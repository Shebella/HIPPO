...
class upgrade_misc_162to2_0_0 extends AbstractUpgradeScript {
	...
	public void upgrade(File oldEucaHome, File newEucaHome) {
		EntityWrapper<ImageInfo> db = new EntityWrapper<ImageInfo>("hippo");
		try {
		...
		//Network rules
		def gen_conn = StandalonePersistence.getConnection("hippo");
		gen_conn.rows('SELECT * FROM metadata_network_group').each{
			EntityWrapper dbGen = new EntityWrapper("hippo");
			try {
		...
		EntityWrapper dbVol = new EntityWrapper("hippo");
