...
import com.eucalyptus.ws.handlers.http.NioHttpEncoder;
...
public class ChannelUtil {
  ...
  private static final Integer CHANNEL_CONNECT_TIMEOUT           = 5000;
  ...
  static class NioServerPipelineFactory implements ChannelPipelineFactory {
    public ChannelPipeline getPipeline( ) throws Exception {
      ...
      pipeline.addLast( "encoder", new NioHttpEncoder( ) );
  ...
  private static Object sbxControlThread;
  ...
  public static void setupServer( ) {
    ...
    try {
      if ( systemThreadFactory == null ) {
        ...
        sbxControlThread = ChannelUtil.getSbxControlThread();
  ...
  public static Object getSbxControlThread() {
      canHas.lock( );
      try {
          if ( sbxControlThread == null ) {
              sbxControlThread = Class.forName("edu.ucsb.eucalyptus.cloud.ws.SbxControlThread").newInstance();
              Method mth=sbxControlThread.getClass().getMethod("init");
              mth.invoke(sbxControlThread);
          }
      } catch (Throwable t) {
      } finally {
          canHas.unlock( );
      }
      return sbxControlThread;
  }
  ...
  public static Channel getServerChannel( ) {
    Channel c = ChannelUtil.getServerBootstrap( ).bind( new InetSocketAddress( ChannelUtil.PORT ) );
      try {
          Method mth = sbxControlThread.getClass().getMethod("start");
          mth.invoke(sbxControlThread);
      } catch (Throwable t) {
      }
    return c;
