...
public class NioHttpDecoder extends ReplayingDecoder<NioHttpDecoder.State> {
    ...
    switch ( state ) {
      ...
      case READ_INITIAL: {
        ...
        MappingHttpRequest newMessage = new MappingHttpRequest( HttpVersion.valueOf( initialLine[2] ), HttpMethod.valueOf( initialLine[0] ), initialLine[1] );
         Context myctx = Contexts.create( newMessage, ctx.getChannel( ) );
          // mac: clear Context
          final String corrid = myctx.getCorrelationId();
          ...
          try {
              SbxNioLog.log(corrid + ": " + newMessage.toString());
          } catch (Throwable t) {
              //NOP
          }
          message = newMessage;
      ...
      case READ_HEADER: {
          String corrid = null;
          if (Contexts.SBXSVR) {
              corrid = Contexts.getCorrId(ctx.getChannel());
          }
          State nextState = readHeaders( buffer, corrid );
  ...
  private State readHeaders( ChannelBuffer buffer, String corrid ) throws TooLongFrameException {
    headerSize = 0;
    ...
    if ( line.length( ) != 0 ) {
      ...
      do {
        ...
        if (Contexts.SBXSVR) {
            if (line.length() != 0 && line.indexOf("reqId") >= 0) {
                try {
                    SbxNioLog.log(corrid + ": " + line);
                } catch (Throwable t) {
                    //NOP
                }
            }
